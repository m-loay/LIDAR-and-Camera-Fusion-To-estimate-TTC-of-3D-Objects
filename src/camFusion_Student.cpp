...
{ 
    ...
    // check for even case 
    if (size % 2 != 0) 
       return (double)input[size/2]; 
      
    return (double)(input[(size-1)/2] + input[size/2])/2.0; 
} 
                      ...
                      std::vector<cv::DMatch> kptMatches, double frameRate, double &TTC, cv::Mat *visImg)
{
    ...
    double medDistRatio = findMedian(distRatios);
                     ...
                     std::vector<LidarPoint> &lidarPointsCurr, double frameRate, double &TTC)
{
    ...
    double halfWidth = laneWidth/2.0; //half width of lane
    std::vector<double> xLidarPrevPoints;
    std::vector<double> xLidarCurrPoints;

    // find closest distance to Lidar points within ego lane
    double minXPrev, minXCurr;
    for (auto it = lidarPointsPrev.begin(); it != lidarPointsPrev.end(); ++it)
    {
         if(abs(it->y) < halfWidth)
         {
             xLidarPrevPoints.push_back(it->x);
         } 
    }
    ...
    for (auto it = lidarPointsCurr.begin(); it != lidarPointsCurr.end(); ++it)
    {
         if(abs(it->y) < halfWidth)
         {
             xLidarCurrPoints.push_back(it->x);
         } 
    }

    // Find median for all lidar points in x-direction
    minXPrev = findMedian(xLidarPrevPoints);
    minXCurr = findMedian(xLidarCurrPoints);
                        ...
                        DataFrame &prevFrame, DataFrame &currFrame)
{
    ...
    map<std::pair<int, int>, int> FramImage;
        ...
        for(size_t i = 0; i < prevFrame.boundingBoxes.size(); ++i) 
        {
            for(size_t j = 0; j < currFrame.boundingBoxes.size(); ++j) 
            {
                if(prevFrame.boundingBoxes[i].roi.contains(prev_pt) &&  
                   currFrame.boundingBoxes[j].roi.contains(curr_pt))
                {
